...
const App = () => {
    ...
    const handleCalcular = () => {
        if (!gas || parseFloat(gas.replace(',', '.')) <= 0 || !eta || parseFloat(eta.replace(',', '.')) <= 0) {
            Alert.alert('Para calcular é necessário informar o preço dos combustíveis!');
        } else {
            let precoGas = parseFloat(gas.replace(',', '.'));
            let precoEta = parseFloat(eta.replace(',', '.'));
            
            let pct = Math.round((precoEta / precoGas) * 100);
            if (pct < 70) {
